...
| R-2026-021 | P-2026-021 | AI heredoc 写入 output.md 验证 | 手工检查 |

---

## R-2026-021 AI heredoc 写入 output.md 验证

- **类型**：手工检查
- **描述**：验证 AI 使用正确语法写入 output.md 后，iterate 弹窗能正确显示任务摘要。
- **检查步骤**：
  1. AI 使用 `<<'MD'`（带引号）或 `write_to_file` 工具写入 `~/.cunzhi/{PORT}/output.md`
  2. 调用 `python3 cunzhi.py {PORT}`
  3. **预期结果**：iterate 弹窗显示写入的任务摘要，而非默认消息"任务完成，请确认是否继续？"
- **失败场景**：
  - 使用 `<<EOF` 且内容包含反引号/特殊字符 → heredoc 不结束，文件为空
  - 终端显示 `cmdand heredoc>` 提示符
- **关联 P-ID**：P-2026-021
- **日期**：2026-01-11
