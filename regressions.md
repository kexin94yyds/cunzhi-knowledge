...
| R-2026-017 | P-2026-017 | 验证 zhi-cli.sh 命令行交互功能 | 手工检查 |
| R-2026-018 | P-2026-018 | Web Bridge 上下文追加验证 | 手工检查 |
| R-2026-019 | P-2026-019 | Tauri 弹窗防覆盖验证 | 手工检查 |
| R-2026-020 | P-2026-020 | Tauri 完整构建白屏验证 | 手工检查 |
...
## R-2026-017 移除相对极差并引入双重 RSD 评估验证

- **类型**: 手工检查 / 逻辑验证
- **描述**: 确认“相对极差”相关内容已彻底移除，且 AI 评估能够正确提取并评价标定与测定两个阶段的 RSD。
- **检查清单**:
  1. [ ] 检查标定页面和测定结果页面，确认所有“相对极差”文字及公式图块已消失。
  2. [ ] 在标定阶段输入一组平行数据，点击计算后进入测定阶段。
  3. [ ] 完成测定并生成报告，查看“AI 智能初审意见”。
  4. [ ] 确认初审意见中是否同时包含了“标定实验精密度 (RSD)”和“测定实验精密度 (RSD)”的数值。
  5. [ ] 验证初审状态（Pass/Review Required）是否基于这两个 RSD 值（阈值均应 ≤ 0.5%）。
  6. [ ] 导出 PDF 报告，确认平均值行中仅显示两个 RSD 值，无相对极差项。
- **状态**: verified
- **日期**: 2026-01-15

---

## R-2026-016 硝酸银标定流程回归检查

- **类型**: 手工检查 / 逻辑验证
- **描述**: 验证标定公式中稀释倍数 Xi 的正确性以及界面简化的交互逻辑。
- **检查清单**:
  1. [ ] 标定页面顶部是否显示 NaCl 质量 m (g)、空白体积 V0 和稀释倍数 Xi 的统一输入框。
  2. [ ] 平行实验一、二、三是否仅保留滴定体积 V 的输入，移除了 NaCl 质量输入。
  3. [ ] 输入数据（m=0.1200, V=20.50, V0=0.05, Xi=200）后，计算出的浓度 c 是否约为 0.0100 mol/L（公式：(0.12*1000)/(58.44*(20.5-0.05)*200)）。
  4. [ ] 导出 PDF 报告，检查原理部分是否包含标定公式，且数据表格中 NaCl 质量显示正确。
- **状态**: verified
- **日期**: 2026-01-15

---

## R-2026-014 实验报告 PDF 导出功能验证

- **类型**：手工检查
- **描述**：验证实验结果页面的“导出报告 (PDF)”按钮是否能正常生成并下载包含报告内容的 PDF 文件。
- **检查步骤**：
  1. 运行应用并进入“数据处理”阶段（WorkflowStage.RESULT）。
  2. 点击“导出报告 (PDF)”按钮。
  3. 观察按钮是否显示“导出中...”及加载图标。
  4. 验证浏览器是否弹出下载对话框，且文件名包含样品名称。
  5. 打开下载的 PDF，检查排版、清晰度以及内容是否完整。
- **关联 P-ID**：P-2026-014
- **日期**：2026-01-10
