...
| R-2026-049 | P-2026-049 | iOS 通知不应重复（mcp_state 去重验证） | 手工检查 |
...
| R-2026-026 | P-2026-026 | YouTube 转录：英文语言 + CC 条件验证 | 手工检查 |
| R-2026-021 | P-2026-021 | Monoshot Web 导入后分类 Tab 刷新仍存在 | 手工检查 |
...
## R-2026-026 YouTube 转录：英文语言 + CC 条件验证

- **关联问题**：P-2026-026
- **类型**：手工检查
- **位置**：YouTube-Transcript 扩展（content script：`content-dom.js`）
- **关键断言**：
  1. YouTube 显示语言设为英文时，存在 CC 的视频能稳定出现 Transcript 入口或被插件抓到字幕。
  2. 无 CC 的视频，插件应明确提示“无可用字幕/无法转录”，而不是误判为代码故障。
- **运行方式**：手工验证
- **步骤**：
  1. 在 YouTube 设置里将语言切换为 English。
  2. 打开一个明确有 CC 的视频（播放器控制条出现 CC 图标）。
  3. 打开插件侧边栏，验证能获取字幕条目数量（日志出现“成功获取 N 条字幕”或侧边栏出现字幕列表）。
  4. 打开一个无 CC 的视频，验证插件给出“无字幕”的明确提示。
...
## R-2026-021 Monoshot Web 导入后分类 Tab 刷新仍存在

- **类型**：手工检查
- **描述**：验证 Web 端导入 JSON 后生成的新分类 Tabs（顶部筛选按钮）在刷新后仍可从本地 IndexedDB 数据中恢复。
- **检查步骤**：
 
   1. 打开 `monoshot-vault` Web 页面。
   2. 通过导入 JSON 功能导入一份包含非默认分类（例如 `coding`/`creating`）的导出文件。
   3. **预期结果**：顶部分类筛选 Tabs 出现新分类按钮。
   4. 进行一次页面刷新（Cmd+R）。
   5. **预期结果**：刷新后顶部 Tabs 仍包含这些新分类按钮（不回退到仅 `DEFAULT_CATEGORIES`）。
   6. 点击这些新分类按钮，确认列表能够正确过滤出对应分类的截图。
- **关联 P-ID**：P-2026-021
- **日期**：2026-01-12
...
## R-2026-027 cunzhi.py 无限等待不超时

- **类型**：手工检查
- **描述**：验证 cunzhi.py 脚本在等待用户响应时不会因超时而中断。
- **检查步骤**：
  1. 启动 iterate 服务：确保端口 5310 或其他端口正在运行
  2. 调用脚本：`python3 bin/cunzhi.py 5310`
  3. **等待超过 5 分钟**（之前的超时时间）
  4. 在 iterate GUI 中输入响应并提交
- **预期结果**：
  - 脚本不会返回 `Error: 请求失败: timed out`
  - 脚本正常返回 `KeepGoing=true` 和用户输入
- **关联 P-ID**：P-2026-027
- **日期**：2026-01-16

---

## R-2026-028 Web 端上下文追加功能验证

- **类型**：手工检查
- **描述**：验证手机端通过 `iterate.tobooks.xin` 访问时，上下文追加功能正常工作。
- **检查步骤**：
  1. 手机访问 `https://iterate.tobooks.xin/`
  2. 等待连接成功（显示"已连接"）
  3. 在桌面端触发一个 MCP 请求
  4. 手机端开启上下文追加开关（如 iterate.zh）
  5. 在输入框输入内容并点击发送
- **预期结果**：
  - 发送的内容应包含上下文追加的模板内容
  - 桌面端收到的 `user_input` 应包含追加的条件性内容
- **关联 P-ID**：P-2026-028
- **日期**：2026-01-16

---

## R-2026-029 iterate 8080 服务自动重启验证

- **类型**：手工检查
- **描述**：验证 iterate 服务崩溃后能自动恢复。
- **检查步骤**：
  1. 确认 watchdog 服务正在运行：`launchctl list | grep iterate`
  2. 确认 8080 端口有服务：`lsof -i :8080`
  3. 强制杀死 iterate 进程：`pkill -9 -f "iterate.app"`
  4. 等待 15 秒
  5. 再次检查 8080 端口：`lsof -i :8080`
- **预期结果**：
  - 服务在 15 秒内自动恢复
  - 日志 `/tmp/iterate-watchdog.log` 显示重启记录
- **关联 P-ID**：P-2026-029
- **日期**：2026-01-16

---

## R-2026-030 iterate 后台运行时快捷键和 Dock 交互验证

- **类型**：手工检查
- **描述**：验证 iterate 以隐藏模式启动后，全局快捷键不干扰其他应用，且可通过 Dock 图标打开主页。
- **检查步骤**：
  1. 杀死 iterate 进程：`pkill -9 -f "iterate.app"`
  2. 等待 watchdog 自动重启（约 15 秒）
  3. 确认 iterate 以隐藏模式启动（主窗口不显示）
  4. 在其他应用中按 `Shift+Tab`
  5. **预期结果**：不触发 iterate 窗口，其他应用正常响应
  6. 点击 Dock 栏中的 iterate 图标
  7. **预期结果**：iterate 主页窗口显示并获得焦点
- **关联 P-ID**：P-2026-030
- **日期**：2026-01-16

---

## R-2026-031 iterate 快捷键禁用窗口隔离验证

- **类型**：手工检查
- **描述**：验证快捷键禁用是窗口级别的，不影响其他窗口，且禁用后全局快捷键不再响应。
- **检查步骤**：
  1. 打开两个 iterate 弹窗（不同项目）
  2. 在窗口 A 中点击禁用快捷键按钮
  3. 最小化窗口 A
  4. 按 `Shift+Tab`
  5. **预期结果**：窗口 A 不被呼出
  6. 检查窗口 B 的快捷键按钮状态
  7. **预期结果**：窗口 B 快捷键仍然启用
  8. 在窗口 B 中按 `Tab` 最小化，再按 `Shift+Tab`
  9. **预期结果**：窗口 B 正常恢复
- **关联 P-ID**：P-2026-031
- **日期**：2026-01-16
...
## R-2026-045 RI 笔记应用 IndexedDB 性能优化验证

- **类型**：手工检查
- **描述**：验证 RI 笔记应用不再出现内存爆炸和 UI 卡顿问题。
- **检查步骤**：
  1. 启动 RI 应用
  2. 点击"全部导出" → "同步到 GitHub"
  3. 观察底部浮动进度提示
  4. 确认进度百分比正常更新
  5. 确认同步完成后显示成功提示
- **预期结果**：
...
## R-2026-046 RI 笔记应用 Git 合并模式验证

- **类型**：手工检查
- **描述**：验证 RI 笔记应用使用合并模式同步，不强制覆盖远程历史。
- **检查步骤**：
  1. 在 GitHub 上手动修改一个笔记文件
  2. 在本地 RI 应用中修改同一笔记
  3. 点击"同步到 GitHub"
  4. 检查 GitHub 上的提交历史
- **预期结果**：
...
## R-2026-047 RI 笔记导出文件名唯一性验证

- **类型**：手工检查
- **描述**：验证导出的笔记文件名使用 ID，不会重复。
- **检查步骤**：
  1. 在同一模式下创建两条相同标题的笔记
  2. 点击"导出到文件系统"
  3. 检查导出的文件
...
## R-2026-049 iOS 通知不应重复（mcp_state 去重验证）

- **类型**：手工检查
- **描述**：验证 Bridge 状态上报不会因重复推送导致 iOS 收到两次相同通知。
- **检查步骤**：
  1. 确保桌面端为最新版本（已执行 `./update.sh` 并重启 `iterate.app`）。
  2. 打开 iOS IterateNotify，确认连接到 WebSocket（状态显示“已连接”）。
  3. 在桌面端触发一次 MCP 请求（任意方式：`iterate --bridge` 或桌面端触发弹窗）。
  4. 观察手机端系统通知与消息列表。
  5. 重复第 3 步 3 次。
- **预期结果**：
  - 每次触发只收到 **1 条** 通知/消息（不应出现同一条消息重复两次）。
- **关联 P-ID**：P-2026-049
- **日期**：2026-01-31

---

## R-2026-050 iOS iterate 应用后台通知验证

- **类型**：手工检查
- **描述**：验证 iOS 应用在后台时能正常接收消息并发送系统通知。
- **检查步骤**：
  1. 在 Xcode 中构建并运行 IterateNotify 应用
  2. 确认应用已连接到 WebSocket 服务器（状态显示"已连接"）
  3. 将应用切到后台（按 Home 键或上滑）
  4. 查看 Xcode 控制台，确认显示 `[App] 进入后台，启动静音音频保持连接`
  5. 发送测试消息：
     ```bash
     curl -X POST http://127.0.0.1:8080/bridge/publish \
       -H "Content-Type: application/json" \
       -d '{"message_type":"mcp_state","payload":{"request":{"message":"后台通知测试"}}}'
     ```
  6. 观察手机是否收到系统通知
- **预期结果**：
  - 控制台显示 `[App] 进入后台，启动静音音频保持连接`
  - 手机收到系统通知横幅
  - 通知内容为"后台通知测试"
- **注意事项**：
  - 确保 iOS 设置 > 通知 > IterateNotify 已开启
  - 确保横幅样式不是"无"
- **关联 P-ID**：P-2026-050
- **日期**：2026-01-29
...
## R-2026-053 静音功能状态同步验证

- **类型**：手工检查
- **描述**：验证静音功能在点击按钮后，新的 MCP 请求能正确响应静音状态（窗口最小化、不发声）。
- **检查步骤**：
  1. 启动 iterate 桌面端应用
  2. 触发一次 MCP 请求，确认弹窗正常显示
  3. 点击顶部工具栏的静音按钮（🔔 → 🔕）
  4. 再次触发 MCP 请求（通过 `iterate --bridge` 或其他方式）
  5. 观察窗口行为
- **预期结果**：
  - 静音按钮状态正确切换（🔕）
  - 新请求到达时窗口最小化到 Dock
