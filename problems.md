...
## P-2026-001 iOS 操作栏按钮与移动端规范不一致

- 项目：iterate (CunZhi)
- 仓库：/Users/apple/cunzhi
- 发生版本：86d9379
- 现象：iOS 端操作栏显示“继续/增强/取消/确认”四个按钮（两行），与移动端仅保留“继续/确认”的规范不一致。
- 根因：iOS Swift UI 沿用桌面端动作入口，未做移动端按钮收敛。
- 修复：移除 iOS 操作栏中的“增强/取消”按钮，调整为一行“继续/确认”双按钮布局。
- 回归检查：待定
- 状态：fixed
- 日期：2026-01-29
...
## P-2026-045 RI 笔记应用 IndexedDB 内存爆炸导致 45GB 内存占用
- **项目**：RI
- **仓库**：https://github.com/kexin94yyds/RI
- **发生版本**：2.1.1
- **现象**：启动应用后内存占用飙升到 45GB，导致系统卡死
- **根因**：
  1. `getWordsByMode` 使用 cursor 逐条读取，性能差
  2. `getDbStats` 加载所有数据来计算数量
  3. `confirm/alert` 阻塞对话框被遮挡导致无响应
  4. 导出循环没有让出事件循环，UI 卡顿
- **修复**：
  1. 使用 `getAll()` 批量读取（快 40-50%）
  2. 使用 `count()` 计算数量
  3. 替换为自定义对话框
  4. 每批处理后 `setTimeout(0)` 让出事件循环
  5. 添加浮动进度提示
- **回归检查**：R-2026-045
- **状态**：verified
- **日期**：2026-01-27
...
## P-2026-046 RI 笔记应用 Git 强制推送覆盖远程历史
- **项目**：RI
- **仓库**：https://github.com/kexin94yyds/RI
- **发生版本**：2.1.1
- **现象**：同步到 GitHub 时使用 `git push -f` 覆盖远程历史，导致数据丢失风险
- **根因**：`pushToRemote` 函数在正常推送失败后使用强制推送
- **修复**：
  1. 使用 `git fetch` + `git pull --rebase` 合并远程更新
  2. 移除所有 `--force` 选项
  3. 首次同步使用 `--allow-unrelated-histories`
- **回归检查**：R-2026-046
- **状态**：verified
- **日期**：2026-01-27
...
## P-2026-047 RI 笔记导出文件名重复导致内容覆盖
- **项目**：RI
- **仓库**：https://github.com/kexin94yyds/RI
- **发生版本**：2.1.1
- **现象**：同一天创建多条相同标题的笔记会被覆盖，更新笔记后日期变化会生成新文件
- **根因**：文件名格式为 `日期-标题.md`，不唯一
- **修复**：使用笔记 ID 作为文件名的一部分：`ID-标题.md`
- **回归检查**：R-2026-047
- **状态**：verified
- **日期**：2026-01-27
...
- **现象**：
  1. 点击静音按钮后，状态显示已切换（🔕）
  2. 但新的 MCP 请求到达时，窗口仍然弹出，没有最小化
  3. 静音功能看起来不生效
- **根因**：
  1. `useMcpHandler()` 每次调用都创建新的 `isMuted` ref 实例
  2. `toggleMute()` 更新的是 UI 组件持有的实例
  3. `showMcpDialog()` 中读取的是事件监听器创建时的另一个实例
  4. 两个实例的 `isMuted.value` 不同步，导致静音检查失效
- **修复**：
  1. 将 `isMuted` 状态提升到模块级别（函数外部）
  2. 使其成为单例，所有调用共享同一个 ref
  3. 确保 `toggleMute()` 和 `showMcpDialog()` 操作同一个状态
- **回归检查**：R-2026-053
- **状态**：open
- **日期**：2026-01-31
- **关键文件**：
