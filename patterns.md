...
## PAT-2026-001: 联动弹出与自动聚焦模式
- **描述**: 在复杂 UI 中，当用户通过特定触发符（如 @）或点击按钮打开子菜单时，应同步实现“向下弹出（防遮挡）”+“自动聚焦输入框”+“关键词监听自动触发”的组合模式，提升交互流畅度。
- **实现要点**:
    1. 使用 `top-full mt-2` 替代 `bottom-full` 以确保在内容较少时向下展开。
    2. 利用 `nextTick` 在菜单渲染后立即调用 `focus()`。
    3. 在父组件监听 `userInput` 变化，实现无缝触发子组件逻辑。
