...
## PAT-2026-054 Windsurf MCP 服务器注册机制

- **问题场景**：
  1. 想要在 Windsurf 中注册自定义 HTTP MCP 服务器
  2. 直接在 `mcp_config.json` 中添加 `serverUrl` 配置
  3. 服务器能正常响应 MCP 协议，但 Windsurf 自动禁用并显示 "Permission denied"
- **模式描述**：
  1. **Windsurf 信任机制**：Windsurf 对直接在配置文件中添加的 HTTP MCP 服务器有安全限制
  2. **扩展注册方式**：Infinite WF 等扩展能工作是因为通过 VS Code 扩展 API 注册 MCP
  3. **技术栈**：使用 `@modelcontextprotocol/sdk` 的 `McpServer` + `StreamableHTTPServerTransport`
- **Infinite WF 实现方式**：
  ```typescript
  // 1. 扩展内部启动 Express HTTP 服务器
  ...
  // 4. Windsurf 通过 serverUrl 连接
  // mcp_config.json: { "my-mcp-5304": { "serverUrl": "http://127.0.0.1:5304/mcp" } }
  ```
- **可选方案**：
  1. **创建 VS Code 扩展**：使用 TypeScript + @modelcontextprotocol/sdk
  2. **使用 --bridge 模式**：通过 `run_command` 调用 `iterate --bridge`（当前方案）
  3. **等待 Windsurf 开放**：未来可能开放自定义 MCP 服务器的信任机制
- **已实现的代码**（供未来使用）：
  - `src/rust/mcp/http_server.rs` - Rust 版 Streamable HTTP MCP 服务器
  - `--mcp-http` CLI 参数 - 启动 HTTP MCP 服务器
  - `src/rust/mcp/handlers/popup.rs` - MCP 工具优先通过 HTTP 调用 `--serve`
- **关联 P-ID**：P-2026-054
- **日期**：2026-02-01
