...
| PAT-2026-017 | MCP 降级策略 | 绕过平台插件系统直接调用 CLI |

---

## PAT-2026-014 React 前端 DOM 导出 PDF 模式

- **场景**: 需要将 React 页面中的特定组件或容器导出为标准 PDF 报告。
- **模式描述**: 
  1. **依赖选型**: 使用 `html2canvas` 截图，`jspdf` 生成 PDF。
  2. **容器定位**: 为目标 DOM 容器设置唯一 ID。
  3. **画布转换**: 配置 `html2canvas` 的 `scale`（推荐 2 以上以保证清晰度）和 `backgroundColor`。
  4. **PDF 适配**: 获取截图比例，自动计算在 A4 (210mm x 297mm) 页面上的宽度和高度。
  5. **加载反馈**: 使用 `isLoading` 状态锁定按钮，避免重复触发，并提供视觉反馈。
- **关联问题**: P-2026-014
- **日期**: 2026-01-10

## PAT-2026-017 关键精密度指标收敛与双重评估模式

- **场景**: 在涉及多个实验阶段（如标定+测定）的定量分析系统中，需要对精密度指标进行精简和一致化评估。
- **模式描述**:
  1. **指标收敛**: 移除冗余的统计指标（如相对极差），统一采用 RSD (相对标准偏差) 作为核心精密度衡量标准，降低用户认知负担。
  2. **双重审计逻辑**: 对于互为基础的实验阶段（标定为测定提供浓度参数），AI 评估应同时覆盖所有阶段的 RSD，确保最终结果的可靠性。
  3. **状态追溯**: 使用独立的状态字段（如 `standardRsd`, `rsd`）分别持久化不同阶段的质量数据，支持结果页面的全景评价。
  4. **UI 纯净化**: 移除与核心计算无关的辅助图块和公式，保持界面专注于关键数据输入与结果产出。
- **关联问题**: P-2026-017
- **日期**: 2026-01-15

---

## PAT-2026-016 复杂实验流程中的参数化标定与界面简化模式

- **场景**: 在定量分析实验（如莫尔法滴定）中，标定过程涉及稀释倍数等修正系数，且多个平行实验共用同一组标准样品（如同一天称取的 NaCl）。
- **模式描述**:
  1. **参数修正**: 在计算公式中显式引入修正系数（如 $Xi$），并提供独立的 UI 输入项，确保计算的严谨性。
  2. **全局输入 vs 个体输入**: 识别多个平行实验中的共同参数（如标准样质量 m），将其提升为全局输入（Top-level Input），避免重复录入和数据不一致风险。
  3. **计算逻辑解耦**: 将修正系数逻辑封装在计算函数内，确保 UI 变更与计算核心逻辑同步。
  4. **透明化展示**: 在结果页面和 PDF 报告中显式标注所使用的修正系数，增强数据的可追溯性和专业度。
- **关联问题**: P-2026-016
- **日期**: 2026-01-15
