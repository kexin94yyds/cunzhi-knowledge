...
主代理分配任务后，**必须自动写入任务队列**，子代理只需输入 `1` 或 `领取` 即可自动获取任务。

**派发流程**：
1. 主代理生成 N 个子代理提示词
2. **调用 `cunzhi_coordinator.py` 写入任务队列**：
   ```bash
   python3 /Users/apple/cunzhi/bin/cunzhi_coordinator.py create "子代理提示词内容"
   ```
3. 通过 `寸止` 通知用户任务已派发
4. 子代理窗口输入 `1` 或 `领取` 自动获取任务
...
for i, batch_prompt in enumerate(prompts):
    os.system(f'python3 /Users/apple/cunzhi/bin/cunzhi_coordinator.py create "{batch_prompt}"')

# 3. 通知用户
print(f"已派发 {len(prompts)} 个任务，子代理输入 '1' 即可领取")
```

**子代理领取**：
- 在其他聊天窗口输入 `1`、`领取`、`go` 等触发词
- `cunzhi_hooks.py` 自动检查队列并注入任务提示词
- 无需复制粘贴
