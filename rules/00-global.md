...
- **闭环行为执行（可选）**：完成三件套沉淀后，询问用户是否需要 Codex 审计。若需要，则提供针对性的 Prompt。若 Codex 审计通过并提供 Diff，AI 助手必须先通过 `zhi` 请求用户确认；确认后再自动应用该 Diff 并完成 Git 同步，将状态从 `fixed` 推进至 `audited (Codex已审计)`。
...
- **盲目生成**：在识别到关键词（如 `git`）时不调用 `ci` 工具而凭记忆输出。
- **静默失败**：工具报错时不告知用户，直接尝试猜测下一步。
- **过度工程**：在审计时建议引入复杂的库或架构，而忽略了“务实极简主义”。

### 规则感知汇报 (Awareness)
每当识别到新的提示词模板（如通过 `git` 触发）或执行关键流程（如收尾审计）时，AI **必须**在回复开头简要声明：“已加载 [规则/模板名]”。这确保用户知晓 AI 正在遵循最新的规范。
...
| "codex" / "audit" | `zhi` | 弹窗输出 `prompts/workflows/audit-with-codex.md` 对应的可复制审计 Prompt（含闭环要求：LGTM → unified diff → `audited`），用于发送给 Codex |
